    ...
    def calc_cost(self, weight):
        ...
        if self.weight_tiers:
            for tier in self.weight_tiers:
                ...
                min_limit = tier.min
                max_limit = tier.max
                if (min_limit is None or weight > min_limit) and (
                    max_limit is None or weight <= max_limit
                ):
...
class ShippingApp:
    def __init__(self, root):
        ...
        style = ttkb.Style(
            "darkly"
        )  # Choose a theme like 'darkly', 'yeti','cosmo',etc.

        # Input frame
        input_frame = ttkb.Frame(root, padding="10")
        input_frame.grid(row=0, column=0, sticky="EW")

        ttkb.Label(input_frame, text="Enter weight (lb):").grid(
            row=0, column=0, sticky="W"
        )
        self.weight_var = tk.StringVar()
        self.weight_entry = ttkb.Entry(
            input_frame, width=10, textvariable=self.weight_var
        )
        ...
        self.calc_button = ttkb.Button(
            button_frame, text="Calculate Cheapest", command=self.calculate_cheapest
        )
    ...
    def calculate_cheapest(self):
        ...
        try:
            ...
            self.result_text.insert(
                tk.END, "Please enter a valid positive number for weight.\n"
            )
